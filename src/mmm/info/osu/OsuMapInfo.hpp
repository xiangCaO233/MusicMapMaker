#ifndef M_OSUMAPINFO_HPP
#define M_OSUMAPINFO_HPP
...
// 通用接口
struct OsuMapChapter {
    virtual ~OsuMapChapter() = default;

    virtual void update_propertie_map(
...
struct OsuMapChapterGeneral : public OsuMapChapter {
    OsuMapChapterGeneral();
    ~OsuMapChapterGeneral() override;
    void update_propertie_map(
        std::unordered_map<MapMetadataType,
                           std::unordered_map<std::string, std::string,
                                              StringHash, std::equal_to<>>>&
            properties) override;
...
struct OsuMapChapterEditor : public OsuMapChapter {
    OsuMapChapterEditor();
    ~OsuMapChapterEditor() override;
    void update_propertie_map(
        std::unordered_map<MapMetadataType,
                           std::unordered_map<std::string, std::string,
                                              StringHash, std::equal_to<>>>&
...
struct OsuMapChapterMetadata : public OsuMapChapter {
    OsuMapChapterMetadata();
    ~OsuMapChapterMetadata() override;
    void update_propertie_map(
        std::unordered_map<MapMetadataType,
                           std::unordered_map<std::string, std::string,
                                              StringHash, std::equal_to<>>>&
     ...
     */
    int32_t BeatmapSetID{-1};
};

struct OsuMapChapterDifficulty : public OsuMapChapter {
    OsuMapChapterDifficulty();
    ~OsuMapChapterDifficulty() override;
    void update_propertie_map(
        std::unordered_map<MapMetadataType,
                           std::unordered_map<std::string, std::string,
                                              StringHash, std::equal_to<>>>&
...
struct OsuMapChapterEvents : public OsuMapChapter {
    OsuMapChapterEvents();
    ~OsuMapChapterEvents() override;
    void update_propertie_map(
        std::unordered_map<MapMetadataType,
                           std::unordered_map<std::string, std::string,
                                              StringHash, std::equal_to<>>>&
...
struct OsuMapChapterColour : public OsuMapChapter {
    OsuMapChapterColour();
    ~OsuMapChapterColour() override;
    void update_propertie_map(
        std::unordered_map<MapMetadataType,
                           std::unordered_map<std::string, std::string,
                                              StringHash, std::equal_to<>>>&
...
class OsuMapMetadata : public MapMetadata {
   public:
    ...
    // 元数据全部章节
    std::unordered_map<std::string, std::unique_ptr<OsuMapChapter>, StringHash,
                       std::equal_to<>>
        chapters;

    // 更新统一属性表
    void update_universe_properties();

    /**
     * @brief 获取特定类型的章节指针
     *
     * @tparam T 想要获取的章节类型 (如 OsuMapChapterGeneral)
     * @param key 章节在 map 中的键 (如 "General")
     *
     * @return T* 如果找到且类型匹配，则返回派生类的裸指针；否则返回 nullptr。
     */
    template <typename T>
    T* get_chapter(std::string_view key) const {
        // 1. 在 map 中查找键
        auto it = chapters.find(key.data());

        // 如果没有找到，安全地返回 nullptr
        if (it == chapters.end()) {
            return nullptr;
        }

        // 2. 从 unique_ptr 获取裸指针，然后使用 dynamic_cast
        // it->second.get() 返回一个 OsuMapChapter*
        return dynamic_cast<T*>(it->second.get());
    }
};

#endif  // M_OSUMAPINFO_HPP
