#ifndef MMM_OSUTIMINGINFO_HPP
#define MMM_OSUTIMINGINFO_HPP
...
// osu时间点元数据类
class OsuTimingMetadata : public TimingMetadata {
   public:
    ...
    // 转换为osu的字符串
    virtual std::string to_osu_description() = 0;

    // 从osu的字符串读取
    virtual void from_osu_description(
        std::vector<std::string>& description) = 0;
};

#endif  // MMM_OSUTIMINGINFO_HPP
