...
class OsuMap : public MMap {
 public:
  ...
  /*
   ***Countdown	Integer（整型）	在第一个物件之前出现的倒计时速度 (0 = 无倒计时,
   *1 = 正常速度, 2 = 一半速度, 3 = 二倍速度)	默认1
   */
  int32_t Countdown{1};
   ...
   *当物件重叠在同一个位置时，决定物件之间是否出现堆叠效果的阈值（0-1）	0.7 Mode
   *Integer（整型）	游戏模式（0 = osu!、1 = osu!taiko、2 = osu!catch、3 =
   *osu!mania）	默认0
   */
  double StackLeniency{0.0};

  /*
   ***LetterboxInBreaks	Boolean（布尔值）是否开启谱面休息段使用黑边填充设置
   *默认0
   */
  ...
  /*
   ***SliderMultiplier	Decimal（精准小数）	基础滑条速度倍率，乘以 100
   *后可得到该速度下每拍内滑条会经过多少 osu! 像素
   * 似乎om模式里没卵用
   */
   ...
   *表示这张背景图片在游玩时，需要移动至屏幕中心向右移动 50 osu! 像素，向下移动
   *100 osu! 像素显示。如果偏移值为 0,0，也可以忽略不写。
   *视频
