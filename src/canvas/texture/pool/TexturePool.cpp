...
void TexturePool::finalize() {
  ...
  if (use_atlas) {
    XWARN("当前纹理池使用纹理集");
    // 初始化纹理数组存储的纹理集数据组
    GLCALL(glGenTextures(1, &atlasMetaTextureArray));
    // 激活纹理单元
    GLCALL(cvs->glActiveTexture(GL_TEXTURE0 + 14));
    GLCALL(glBindTexture(GL_TEXTURE_2D_ARRAY, atlasMetaTextureArray));

    // 创建32宽×17高×N层的RGBA32F纹理数组
    GLCALL(glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA32F, 32, 17,
