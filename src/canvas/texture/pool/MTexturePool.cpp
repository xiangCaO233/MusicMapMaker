...
MTexturePool::MTexturePool(GLCanvas *canvas, uint32_t layer_width,
    ...
    if (sampler_unit != -1) {
        ...
        */
        ...
        GLCALL(cvs->glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA32F, 32, 17,
                                 current_max_layer, 0, GL_RGBA, GL_FLOAT,
                                 nullptr));
#endif  //__APPLE__
    ...
    for (int i = 0; i < current_max_layer; i++) {
        std::vector<float> atlas_meta_head = {
            float(i), 512.0f, float(layer_width), float(layer_height)};
...
bool MTexturePool::load_texture(std::shared_ptr<TextureInstace> &texture) {
    ...
    GLCALL(cvs->glBindTexture(GL_TEXTURE_2D_ARRAY,
                              current_gl_texture_array_handle));
    // draw_texture_debug_border(texture->data, texture->width,
    // texture->height);
    ...
    // XINFO("纹理:[" + std::to_string(texture->texture_id) + "]" +
    // texture->name
    // +
    //       "已上传到gpu");
    ...
    std::vector<float> metadata = {
        float(texture->woffset), float(texture->hoffset), float(texture->width),
...
void MTexturePool::use(MTexturePool *&pool_reference,
    ...
    if (need_update) {
        ...
        GLCALL(cvs->glBindTexture(GL_TEXTURE_2D_ARRAY, atlasMetaTextureArray));
        // 更新sampler uniform
        renderer_context->shader->set_sampler(
            "atlas_meta_buffer_array", max_sampler_consecutive_count - 1);
    }
}
