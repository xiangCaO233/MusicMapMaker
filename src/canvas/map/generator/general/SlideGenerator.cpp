...
void SlideGenerator::generate(Slide& slide) {
    ...
    if (slide.slide_parameter > 0) {
        ...
        // 左滑,矩形整体左移矩形宽度的位置
        horizon_body_pos_x = horizon_body_pos_x + head_cp.x() -
                             horizon_body_width + horizon_body_height / 2.0;
        slide_end_texture = editor_ref->canvas_ref->skin.get_object_texture(
        ...
        slide_end_selected_texture =
            editor_ref->canvas_ref->skin.get_object_texture(
                TexType::SLIDE_END_LEFT, ObjectStatus::SELECTED);
    }
    ...
    // 箭头位置--滑键结束轨道的位置
    slide_end_rect =
        QRectF(horizon_body_end_pos_x - slide_end_size.width() / 2.0,
...
void SlideGenerator::generate_preview(Slide& slide) {
    ...
    auto horizon_body_end_pos_x =
        ...
        editor_ref->ebuffer.preview_orbit_width * endorbit +
    ...
    auto slide_end_size =
        QSizeF(slide_end_texture->width *
                   editor_ref->ebuffer.preview_object_size_scale * 0.75 * 0.75 *
                   editor_ref->canvas_ref->working_map->project_reference
                       ->config.object_width_ratio,
               slide_end_texture->height *
                   editor_ref->ebuffer.preview_object_size_scale * 0.75 * 0.75 *
    ...
    preview_shape_queue.emplace(
        slide_hori_body_rect.x(), slide_hori_body_rect.y(),
        slide_hori_body_rect.width(), slide_hori_body_rect.height(),
        slide_hori_body_texture, objref);
    ...
    switch (slide.compinfo) {
        case ComplexInfo::NONE: {
            ...
            // 直接添加滑尾图形到队列
            preview_shape_queue.emplace(
                slide_end_rect.x(), slide_end_rect.y(), slide_end_rect.width(),
                slide_end_rect.height(), actual_use_end_texture, objref);

            // 滑头--直接将NoteGenerator中结果enqueue
            NoteGenerator::preview_object_enqueue();
            break;
        }
        case ComplexInfo::HEAD: {
            // 多画个头-在尾处追加一个节点
            NoteGenerator::preview_object_enqueue();
            preview_temp_node_map.try_emplace(slide_ptr, node_bound);
            break;
        }
        case ComplexInfo::BODY: {
            // 仅在尾处追加一个节点
        ...
        case ComplexInfo::END: {
            // 补齐节点,画个尾
            // 滑键是否完全过了当前时间
            dump_nodes_to_preview_queue(slide.timestamp <
                                        editor_ref->cstatus.current_time_stamp);
