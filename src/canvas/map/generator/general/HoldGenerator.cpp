...
void HoldGenerator::generate(Hold &hold) {
    ...
    auto hold_ptr = std::shared_ptr<Hold>(&hold, [](Hold *) {});
    // 面条使用面尾标识
    ...
    auto &long_note_end_texture =
        editor_ref->canvas_ref->skin.get_object_texture(TexType::HOLD_END,
                                                        ObjectStatus::COMMON);
    ...
    auto long_note_end_size = QSizeF(
        long_note_end_texture->width * editor_ref->ebuffer.object_size_scale *
            editor_ref->canvas_ref->working_map->project_reference->config
                .object_width_ratio,
        long_note_end_texture->height * editor_ref->ebuffer.object_size_scale *
            editor_ref->canvas_ref->working_map->project_reference->config
                .object_height_ratio);
    ...
    shape_queue.emplace(hold_vert_body_rect.x(), hold_vert_body_rect.y(),
                        hold_vert_body_rect.width(),
                        hold_vert_body_rect.height(), hold_vert_body_texture,
                        objref);
...
void HoldGenerator::generate_preview(Hold &hold) {
    ...
    preview_shape_queue.emplace(
        hold_vert_body_rect.x(), hold_vert_body_rect.y(),
    ...
    switch (hold.compinfo) {
        case ComplexInfo::NONE: {
            ...
            // 直接添加面尾图形到队列
            preview_shape_queue.emplace(
                hold_end_rect.x(), hold_end_rect.y(), hold_end_rect.width(),
                hold_end_rect.height(), hold_end_texture, objref);
        ...
        case ComplexInfo::HEAD: {
            ...
            // 先把面头enqueue-并在尾处追加一个节点
            NoteGenerator::preview_object_enqueue();
            preview_temp_node_map.try_emplace(hold_ptr, node_bound);
            break;
        }
        case ComplexInfo::BODY: {
            // 仅在尾处追加一个节点
        ...
        case ComplexInfo::END: {
            ...
            // 面条是否完全过了当前时间
            dump_nodes_to_preview_queue(hold.hold_end_reference->timestamp <
                                        editor_ref->cstatus.current_time_stamp);
