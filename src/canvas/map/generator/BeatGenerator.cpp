...
void BeatGenerator::generate() {
    for (int i = 0; i < editor_ref->ebuffer.current_beats.size(); ++i) {
        ...
        if (!beat->divisors_customed) {
            // 未单独调节过分拍策略或是读取的
            // 应用默认分拍策略
            beat->divisors = editor_ref->canvas_ref->working_map
                                 ->project_reference->config.default_divisors;
        }

        // 每拍时间*时间线缩放=拍距
        double beat_distance = 60.0 / beat->bpm * 1000.0 *
            ...
            if (divisor_pos >= -beat_distance &&
                ...
                if (divisor_time >=
                    ...
                    if (j == 0) {
                        ...
                        if (divinfos.empty()) {
                            divinfo = std::pair<QColor, int32_t>(
                                QColor(128, 128, 128, 255), 2);
                        } else {
        ...
        text_queue.emplace(
            str_bound.x() +
                editor_ref->canvas_ref->skin.timeinfo_font_size / 2.0,
            str_bound.y() +
                editor_ref->canvas_ref->skin.timeinfo_font_size / 2.0 +
                divstr_height,
            divinfo);
