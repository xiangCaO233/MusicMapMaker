...
StaticRenderer::StaticRenderer(GLCanvas* canvas,
    ...
    // 圆角半径信息
    // 描述location8
    // 顶点缓冲11~11float为float类型数据--圆角半径信息(用float接收)
...
void StaticRenderer::update_gpu_memory() {
    ...
    for (const auto& [instance_start_index, instance_count] : update_list) {
        ...
        for (int i = instance_start_index;
            ...
            //// 图形位置数据
            memory_block[(i - instance_start_index) * 12] =
                position_data[i].x();
            memory_block[(i - instance_start_index) * 12 + 1] =
                position_data[i].y();
            //// 图形尺寸
            memory_block[(i - instance_start_index) * 12 + 2] =
                size_data[i].x();
            memory_block[(i - instance_start_index) * 12 + 3] =
                size_data[i].y();
            //// 旋转角度
            memory_block[(i - instance_start_index) * 12 + 4] =
                rotation_data[i];
            //// 贴图方式
