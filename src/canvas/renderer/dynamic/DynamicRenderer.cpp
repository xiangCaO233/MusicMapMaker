...
DynamicRenderer::DynamicRenderer(GLCanvas* canvas,
    ...
    GLCALL(cvs->glBufferData(GL_ARRAY_BUFFER,
                             (int)(max_shape_count * 2 * sizeof(float)),
    ...
    GLCALL(cvs->glVertexAttribPointer(4, 1, GL_FLOAT, GL_FALSE, sizeof(float),
                                      nullptr));
    GLCALL(cvs->glBufferData(GL_ARRAY_BUFFER,
                             (int)(max_shape_count * 1 * sizeof(float)),
                             nullptr, GL_DYNAMIC_DRAW));
    ...
    GLCALL(cvs->glBufferData(GL_ARRAY_BUFFER,
                             (int)(max_shape_count * 4 * sizeof(float)),
                             nullptr, GL_DYNAMIC_DRAW));
    ...
    switch (data_type) {
        ...
        case InstanceDataType::TEXTURE_ID: {
            ...
            if (texture_id_data.empty() ||
                texture_id_data.size() <= instance_index) {
                // XWARN("添加纹理id数据");
...
void DynamicRenderer::synchronize_update_mark(InstanceDataType data_type,
    ...
    switch (data_type) {
        case InstanceDataType::POSITION: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit = update_mapping.try_emplace(InstanceDataType::POSITION)
                             .first;
            }
            // 更新连续更新标记映射
        ...
        case InstanceDataType::SIZE: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit =
                    update_mapping.try_emplace(InstanceDataType::SIZE).first;
            }
            // 更新连续更新标记映射
            mark_list = &listit->second;
            break;
        ...
        case InstanceDataType::ROTATION: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit = update_mapping.try_emplace(InstanceDataType::ROTATION)
                             .first;
            }
            // 更新连续更新标记映射
            mark_list = &listit->second;
        ...
        case InstanceDataType::TEXTURE_POLICY: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit =
                    update_mapping.try_emplace(InstanceDataType::TEXTURE_POLICY)
                        .first;
            }
            // 更新连续更新标记映射
            mark_list = &listit->second;
        ...
        case InstanceDataType::TEXTURE_ID: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit =
                    update_mapping.try_emplace(InstanceDataType::TEXTURE_ID)
                        .first;
            }
            // 更新连续更新标记映射
            mark_list = &listit->second;
        ...
        case InstanceDataType::FILL_COLOR: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit =
                    update_mapping.try_emplace(InstanceDataType::FILL_COLOR)
                        .first;
            }
            // 更新连续更新标记映射
            mark_list = &listit->second;
        ...
        case InstanceDataType::RADIUS: {
            ...
            if (listit == update_mapping.end()) {
                // 添加缓冲区映射并更新迭代器
                listit =
                    update_mapping.try_emplace(InstanceDataType::RADIUS).first;
            }
            // 更新连续更新标记映射
            mark_list = &listit->second;
            break;
    ...
    if (mark_list->empty()) {
        ...
        // 非空,检查是否连续上一更新标记
        if (mark_list->back().first + mark_list->back().second ==
            instance_index) {
            // 连续--增加更新数量
            ...
            switch (data_type) {
                case InstanceDataType::POSITION: {
                    ...
                    for (int i = instance_start_index;
                        ...
                        memory_block[2 * (i - instance_start_index)] =
                            position_data[i].x();
                ...
                case InstanceDataType::ROTATION: {
                    ...
                    for (int i = instance_start_index;
                         i < instance_start_index + instance_count; i++)
                        memory_block[(i - instance_start_index)] =
                            rotation_data[i];
                    break;
                }
                case InstanceDataType::TEXTURE_POLICY: {
