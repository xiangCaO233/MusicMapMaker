...
class RendererManager {
   ...
   public:
    ...
    // 添加文本
    void addText(const QPointF& pos, const std::u32string& text,
    ...
    // 添加椭圆
    void addEllipse(const QRectF& bounds,
                    std::shared_ptr<TextureInstace> texture,
                    const QColor& fill_color, float rotation, bool is_volatile);
    ...
    // 设置uniform浮点
    void set_general_uniform_float(const char* location_name,
                                   float value) const;
    void set_fontpool_uniform_float(const char* location_name,
                                    float value) const;
