...
GlyphPacker::GlyphPacker(int width, int height) {
  binWidth = width;
  binHeight = height;
...
int GlyphPacker::Insert(CharacterGlyph *glyph, FreeRectChoiceHeuristic method) {
  auto res = Insert(glyph->size.width(), glyph->size.height(), method);
  if (res.height == 0) {
    return -1;
  } else {
    // 应用插入结果
    glyph->pos_in_atlas.setX(res.x);
    glyph->pos_in_atlas.setY(res.y);
    return 1;
  }
}
// 移除字形
int GlyphPacker::Erase(CharacterGlyph *glyph) {
  Rect r;
  r.x = glyph->pos_in_atlas.x();
  r.y = glyph->pos_in_atlas.y();
  r.width = glyph->size.width();
  r.height = glyph->size.height();
  auto res = usedRectangles.begin();
  for (auto it = usedRectangles.begin(); it != usedRectangles.end(); ++it)
    if (*it == r) res = it;
  if (res != usedRectangles.begin() && res != usedRectangles.end()) {
    usedRectangles.erase(res);
    return 1;
  }
  return -1;
}

Rect GlyphPacker::Insert(int width, int height,
...
void GlyphPacker::Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst,
                         FreeRectChoiceHeuristic method) {
  dst.clear();
...
void GlyphPacker::PlaceRect(const Rect &node) {
  size_t numRectanglesToProcess = freeRectangles.size();
  for (size_t i = 0; i < numRectanglesToProcess; ++i) {
...
Rect GlyphPacker::ScoreRect(int width, int height,
...
float GlyphPacker::Occupancy() const {
...
Rect GlyphPacker::FindPositionForNewNodeBottomLeft(int width, int height,
                                                   int &bestY,
                                                   int &bestX) const {
  Rect bestNode;
...
Rect GlyphPacker::FindPositionForNewNodeBestShortSideFit(
...
Rect GlyphPacker::FindPositionForNewNodeBestLongSideFit(
    int width, int height, int &bestShortSideFit, int &bestLongSideFit) const {
  Rect bestNode;
  memset(&bestNode, 0, sizeof(Rect));
...
Rect GlyphPacker::FindPositionForNewNodeBestAreaFit(
...
int GlyphPacker::CommonIntervalLength(int i1start, int i1end, int i2start,
                                      int i2end) const {
  if (i1end < i2start || i2end < i1start) return 0;
  return min(i1end, i2end) - max(i1start, i2start);
}

int GlyphPacker::ContactPointScoreNode(int x, int y, int width,
                                       int height) const {
  int score = 0;
...
Rect GlyphPacker::FindPositionForNewNodeContactPoint(
...
bool GlyphPacker::SplitFreeNode(Rect freeNode, const Rect &usedNode) {
...
void GlyphPacker::PruneFreeList() {
