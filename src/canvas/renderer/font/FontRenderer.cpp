...
FontRenderer::FontRenderer(GLCanvas* canvas,
    ...
    GLCALL(
        cvs->glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, stride, nullptr));
    // 每个实例变化一次
    ...
    // 纹理集层数索引
    // 描述location5
    // 缓冲位置5~5float为float类型数据--纹理集层数索引(用float接收)
    ...
    for (int i = 0; i < 4; i++) {
        ...
        GLCALL(
            cvs->glVertexAttribPointer(8 + i, 2, GL_FLOAT, GL_FALSE, stride,
                                       (void*)((11 + 2 * i) * sizeof(float))));
        GLCALL(cvs->glVertexAttribDivisor(8 + i, 1));
    }

    GLCALL(cvs->glBufferData(GL_ARRAY_BUFFER, (max_shape_count * stride),
    ...
    for (const auto& c : str) {
        ...
        if (cit == pack.character_set.end()) {
            ...
            if (FT_Load_Char(face, c, FT_LOAD_RENDER)) {
                ...
                int glyphDescend = (face->glyph->metrics.height -
                                    face->glyph->metrics.horiBearingY) /
                                   64;
                maxRowHeight =
                    std::max(maxRowHeight, glyphHeight + glyphDescend);

                // 上传字符位图到纹理

                GLCALL(cvs->glActiveTexture(GL_TEXTURE0 + 13));
                GLCALL(cvs->glBindTexture(GL_TEXTURE_2D_ARRAY,
                                          glyphs_texture_array));
                GLCALL(cvs->glTexSubImage3D(
                    GL_TEXTURE_2D_ARRAY, 0, currentX, currentY,
                    free_layer_index, bitmap.width, bitmap.rows, 1, GL_RED,
    ...
    switch (data_type) {
        case InstanceDataType::POSITION: {
            ...
            if (position_data.empty() ||
                position_data.size() <= instance_index) {
                // XWARN("添加位置数据");
        ...
        case InstanceDataType::ROTATION: {
            ...
            if (rotation_data.empty() ||
                rotation_data.size() <= instance_index) {
                // XWARN("添加角度数据");
        ...
        case InstanceDataType::FILL_COLOR: {
            ...
            if (fill_color_data.empty() ||
                fill_color_data.size() <= instance_index) {
                // XWARN("添加填充颜色数据");
        ...
        case InstanceDataType::TEXT: {
            ...
            CharacterGlyph glyph;
            get_character_glyph(text->font_family, text->font_size,
            ...
            if (bearing_y_data.empty() ||
                bearing_y_data.size() <= instance_index) {
                // XWARN("添加y保留高度数据");
...
void FontRenderer::update_gpu_memory() {
    ...
    for (const auto& [instance_start_index, instance_count] : update_list) {
        ...
        GLCALL(cvs->glBufferSubData(
            GL_ARRAY_BUFFER,
            (instance_start_index * float_stride * sizeof(float)),
