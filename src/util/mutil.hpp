...
// 1. 用于存储每行排序所需数据的结构体
struct RowData {
    int priority1_numericValue;  // 优先级1：来自QLineEdit的数字
    int priority2_comboIndex;    // 优先级2：来自QComboBox的索引
    int originalRow;             // 该数据来自原始的哪一行
};
...
// 辅助函数：从指定行“取出”所有内容 (已修正)
inline RowContent takeRow(QTableWidget* table, int row) {
    RowContent content;
    content.items.resize(table->columnCount());
    content.widgets.resize(table->columnCount());

    for (int col = 0; col < table->columnCount(); ++col) {
        // 取出 QTableWidgetItem (这部分没问题)
        content.items[col] = table->takeItem(row, col);
        ...
        if (widget) {
            ...
            // 将获取到的指针存起来
            content.widgets[col] = widget;
        } else {
            content.widgets[col] = nullptr;
        }
    }
    return content;
}
...
// 辅助函数：将一行内容“放回”到指定行
inline void setRow(QTableWidget* table, int row, const RowContent& content) {
    for (int col = 0; col < table->columnCount(); ++col) {
        if (content.items[col]) {
            table->setItem(row, col, content.items[col]);
        }
        if (content.widgets[col]) {
            table->setCellWidget(row, col, content.widgets[col]);
        }
    }
}

// 核心排序函数 (无需修改)
inline void sortTableByCustomLogic(QTableWidget* tableWidget) {
    if (tableWidget->rowCount() <= 1) return;

    // 步骤 1: 数据提取
    QVector<RowData> sortableData;
    for (int row = 0; row < tableWidget->rowCount(); ++row) {
        RowData data;
        data.originalRow = row;
        auto* lineEdit =
            qobject_cast<TimeEditWidget*>(tableWidget->cellWidget(row, 0))
                ->timeEdit;
        data.priority1_numericValue = lineEdit ? lineEdit->text().toInt() : 0;
        QComboBox* comboBox =
            qobject_cast<QComboBox*>(tableWidget->cellWidget(row, 1));
        data.priority2_comboIndex = comboBox ? comboBox->currentIndex() : -1;
        sortableData.append(data);
    }

    // 步骤 2: 自定义排序
    std::sort(sortableData.begin(), sortableData.end(),
              [](const RowData& a, const RowData& b) {
                  if (a.priority1_numericValue != b.priority1_numericValue) {
                      return a.priority1_numericValue <
                             b.priority1_numericValue;
                  }
                  return a.priority2_comboIndex < b.priority2_comboIndex;
              });

    // 步骤 3: 重排表格行
    QVector<RowContent> allRowsContent;
    // 这里必须从后往前 take，因为 removeRow 会改变行号
    for (int row = tableWidget->rowCount() - 1; row >= 0; --row) {
        allRowsContent.prepend(takeRow(tableWidget, row));
    }

    // 清空行后重新设置
    tableWidget->setRowCount(0);
    tableWidget->setRowCount(sortableData.size());

    for (int i = 0; i < sortableData.size(); ++i) {
        int originalRowIndex = sortableData[i].originalRow;
        setRow(tableWidget, i, allRowsContent[originalRowIndex]);
    }
}
