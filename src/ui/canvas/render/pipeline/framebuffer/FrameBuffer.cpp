...
FrameBuffer::~FrameBuffer() {
  // 删除全部纹理附件
  GLCALL(
      cvs->glDeleteTextures(color_attachments.size(), color_attachments.data()),
      cvs);
  // 删除fbo
...
uint32_t FrameBuffer::add_color_attachment() {
  ...
  GLCALL(cvs->glGenTextures(
             1, color_attachments.data() + color_attachments.size() - 1),
         cvs);
  auto& texture = color_attachments.back();
  ...
  GLCALL(
      cvs->glFramebufferTexture2D(
          GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + color_attachments.size() - 1,
          GL_TEXTURE_2D, texture, 0),
      cvs);
  release();
  return color_attachments.back();
}
