...
void Renderer2D::initShader(QOpenGLShaderProgram*& shader,
  ...
  if (geometry_shader_source_path) {
    ...
    if (!shader->addShaderFromSourceCode(QOpenGLShader::Geometry,
      ...
      qCritical() << "Renderer " << debug_name
                  << " Geometry Shader compilation failed:" << shader->log();
    }
  }
...
void Renderer2D::initShaderUBO(QOpenGLShaderProgram*& shader,
  ...
  if (GLuint mask_ubo_index = GLCALL(
    ...
    qWarning() << "Could not find uniform block '" << name_in_shader << "' in "
               << debug_name << " shader program.";
  }
}
...
void Renderer2D::drawBatch(const RenderBatch& batch,
  ...
  switch (batch.type) {
    ...
    case QUAD: {
      ...
      DRAWCALL(
          cvs->glDrawArraysInstanced(GL_TRIANGLES, 0, 6, batch.elementCount),
          cvs);
      if (wireframe) {
        shader->setUniformValue("u_IsDrawingWireframe", true);
        DRAWCALL(
            cvs->glDrawArraysInstanced(GL_LINE_LOOP, 0, 6, batch.elementCount),
            cvs);
      }
      break;
    }
    case PRIMITIVE: {
      shader->setUniformValue("u_IsDrawingWireframe", false);
      DRAWCALL(
          cvs->glDrawArrays(GL_POINTS, batch.startIndex, batch.elementCount),
          cvs);
      if (wireframe) {
        shader->setUniformValue("u_IsDrawingWireframe", true);
    ...
    case CURVE: {
      shader->setUniformValue("u_IsDrawingWireframe", false);
      DRAWCALL(
          cvs->glDrawArrays(GL_TRIANGLES, batch.startIndex, batch.elementCount),
          cvs);
      if (wireframe) {
        shader->setUniformValue("u_IsDrawingWireframe", true);
        DRAWCALL(cvs->glDrawArrays(GL_LINE_LOOP, batch.startIndex,
