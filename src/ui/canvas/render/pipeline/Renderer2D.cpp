...
void Renderer2D::commit(const QuadCommand& command) {
  {
    ...
    quad_datas.emplace_back(
        ...
        command.radiusInfo.radius_effect, command.texturesInfo.texture.uv_scale,
        command.texturesInfo.texture.group_size,
        command.texturesInfo.texture.layer_index, command.baseInfo.no_filter,
        command.texturesInfo.talign, command.texturesInfo.tscale,
        PrimitiveType::QUAD);
  }
}
...
void Renderer2D::finalize() {
  ...
  switch (first_cmd.cmdType) {
    ...
    case MESH: {
      elementCount = static_cast<const MeshCommand&>(first_cmd).indicies.size();
  ...
  for (size_t i = 1; i < all_command_handles.size(); ++i) {
    ...
    if (cmd_emergable(command, command_batchs)) {
      ...
      switch (command.cmdType) {
        ...
        case MESH: {
          command_batchs.back().elementCount +=
              static_cast<const MeshCommand&>(command).indicies.size();
          break;
        }
        case CURVE: {
          command_batchs.back().elementCount +=
              static_cast<const CurveCommand&>(command).vertices.size();
          break;
        }
      }
    } else {
      ...
      command_batchs.emplace_back(
          command.cmdType, command.texturesInfo.texture.gl_texture_array_id,
          // ★ 使用类型特定的实例索引
