...
TexturePool::TexturePool(QOpenGLFunctions_4_1_Core* gl_functions)
  ...
  // 查询最大支持多层纹理的最大层数
  // 查询硬件支持
  GLCALL(glGetIntegerv(GL_MAX_ARRAY_TEXTURE_LAYERS, &max_texture_array_layers),
         glf);
  qDebug() << "多层纹理最大层数: " << std::to_string(max_texture_array_layers);
...
void TexturePool::buildFromManifest(
  ...
  for (const auto& path : texture_paths) {
    if (stbi_info(path.c_str(), &w, &h, &c)) {
      ...
      std::string bucket_key = std::format("Bucket_{}x{}", bucket_w, bucket_h);
  ...
  for (const auto& [bucket_key, paths_in_bucket] : buckets) {
    ...
    for (size_t i = 0; i < paths_in_bucket.size();
      ...
      std::string group_key = bucket_key + "_Part" + std::to_string(part_index);
      ...
      GLCALL(glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA8, group.bucket_width,
                          group.bucket_height, group.layer_count, 0, GL_RGBA,
      ...
      for (size_t layer_idx = 0; layer_idx < chunk_paths.size(); ++layer_idx) {
        ...
        threadpool.enqueue([this, path, info]() {
          ...
          if (data) {
            LoadedImageData image_data{path, load_w, load_h, 4, data};
    ...
    for (const auto& [_, group] : atlas_groups) {
      auto& internal_group = groups.try_emplace(group.gl_id).first->second;
