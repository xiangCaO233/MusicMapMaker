...
void FontPool::load_font(std::string_view font_path, bool is_qrc) {
  ...
  for (const auto& font_size_to_load : font_sizes) {
    threadpool.enqueue([this, font_path_str = std::string(font_path), is_qrc,
                        font_size_to_load]() {
      ...
      if (is_qrc) {
        ...
        if (font_file.open(QIODevice::ReadOnly)) {
          ...
          if (FT_New_Memory_Face(
                  thread_local_library,
                  reinterpret_cast<const FT_Byte*>(font_data.constData()),
                  font_data.size(), 0,
      ...
      for (char32_t c : cjkstr) {
        ...
        if (FT_Load_Char(face, c, FT_LOAD_RENDER)) {
          qWarning() << "FontPool: Failed to load glyph for char:" << (uint)c;
          continue;
        } else {
          // 成功加载了字形
          // qDebug()
        ...
        for (int y = 0; y < height; ++y) {
          memcpy(single_channel_data + y * width,     // 目标地址：紧凑布局
                 bitmap->buffer + y * bitmap->pitch,  // 源地址：带pitch的布局
                 width                                // 拷贝数量：只有有效宽度
          );
        }
        // for (int y = 0; y < height; ++y) {
        ...
        LoadedImageData imageData{
            face->family_name, font_size_to_load, c, width, height,
            single_channel_data,
...
void FontPool::uploadToGpu(const LoadedImageData& data) {
  ...
  if (current_atlas_id == 0) {
    ...
    GLCALL(glf->glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_R8, LAYER_SIZE.x,
                             LAYER_SIZE.y, max_texture_array_layers, 0, GL_RED,
  ...
  if (current_layer_index >= max_texture_array_layers) {
    ...
    qWarning() << "FontPool: Texture array is full! Need to create a new one.";
