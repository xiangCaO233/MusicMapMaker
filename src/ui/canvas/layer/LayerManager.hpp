...
class LayerManager {
   public:
    ...
    LayerManager() {
        ...
        //     data_buffer.try_emplace(BACKGROUND).first->second.get();
        // auto timeline_layer =
        //     data_buffer.try_emplace(TIMELINE).first->second.get();
        // auto note_layer = data_buffer.try_emplace(NOTE).first->second.get();
        // auto effect_layer =
        // data_buffer.try_emplace(EFFECT).first->second.get();
        auto interact_layer =
            data_buffer
        ...
        //     generators
        //         .try_emplace(BACKGROUND, std::make_unique<LayerComputerBase>(
        //                                      background_layer,
        //                                      &synchronizer))
        //         .first->second.get();
        ...
        //     generators
        //         .try_emplace(TIMELINE, std::make_unique<LayerComputerBase>(
        //                                    timeline_layer, &synchronizer))
        //         .first->second.get();

        // auto note_generator =
        //     generators
        //         .try_emplace(NOTE, std::make_unique<LayerComputerBase>(
        //                                note_layer, &synchronizer))
        //         .first->second.get();

        // auto effect_generator =
        //     generators
        //         .try_emplace(EFFECT, std::make_unique<LayerComputerBase>(
        ...
        // startGenerator(background_generator);
        // startGenerator(timeline_generator);
        // startGenerator(note_generator);
        // startGenerator(effect_generator);
   ...
   private:
    ...
    // 持有帧同步器
    FrameSynchronizer synchronizer{1};
