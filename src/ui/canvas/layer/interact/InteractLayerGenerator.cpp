...
// --- 1. 效果参数 ---
const int NUM_RINGS = 8;             // 组成光晕的同心圆数量
const float BASE_RADIUS = 4.0f;      // 核心光点的半径
const float WAVE_AMPLITUDE = 22.0f;  // 呼吸波动的最大半径
const float WAVE_SPEED = 2.f;        // 呼吸的速度
...
void InteractLayerGenerator::generateLayer(LayerManager* manager,
    ...
    if (pressed) {
        ...
        for (int i = 0; i < NUM_RINGS; ++i) {
            ...
            cmd.baseInfo.color.a =
                (1.0f - pulse) * 0.7f;  // [0, 0.7]，越向外越透明
