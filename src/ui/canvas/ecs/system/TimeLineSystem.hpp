   ...
   public:
    void update(ECSCore& core, const MapCanvasInfo* info,
        ...
        for (auto it = beats.rbegin(); it != beats.rend(); ++it) {
            ...
            if (!registry.all_of<InMaintrackComponent>(current_beat_entity))
                continue;
            const auto& [time] =
            ...
            if (y < all_tracks_rect.y ||
                ...
                cmd.baseInfo.size = {all_tracks_rect.x + all_tracks_rect.z, 6};
        ...
        // 遍历所有可见的timing实体
        auto view = registry.view<TimingComponent, InMaintrackComponent>();
