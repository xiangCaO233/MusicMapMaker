...
UVResult calcUV() {
    ...
    if (fs_in.f_TexScaleStratergy != CHARACTER && (fs_in.f_UVScale.x == 0.0 || fs_in.f_UVScale.y == 0.0 || fs_in.f_GroupSize.x == 0.0 || fs_in.f_GroupSize.y == 0.0)) {
    ...
    if (fs_in.f_TexScaleStratergy != CHARACTER) {
        actualTexSize = fs_in.f_GroupSize * fs_in.f_UVScale;
        fragPosInQuad = fs_in.v_TexCoord * fs_in.f_QuadSize;
    }

    switch (fs_in.f_TexScaleStratergy) {
        case AUTO_SCALE_AND_CUT:
        {
            // 计算宽高缩放比
            vec2 scaleRatios = fs_in.f_QuadSize / actualTexSize;
            // 选择较大的缩放比作为最终的统一缩放因子
            float finalScale = max(scaleRatios.x, scaleRatios.y);
            // 计算缩放后纹理的尺寸
            vec2 scaledTexSize = actualTexSize * finalScale;
            // 计算居中对齐的偏移量
            vec2 offset = (fs_in.f_QuadSize - scaledTexSize) / 2.0;
            // 计算在源纹理上的采样像素坐标
            ...
            result.uv = tiledUV * fs_in.f_UVScale;
            break;
        }
        case SCALE_FORCE_BASEWIDTH:
        {
            // 计算宽度基准的缩放因子
            float finalScale = fs_in.f_QuadSize.x / actualTexSize.x;
            // 计算缩放后纹理的尺寸
            vec2 scaledTexSize = actualTexSize * finalScale;
            // 根据对齐策略，计算垂直方向的偏移量
            float offsetY = 0.0;
            if (fs_in.f_TexAlignStratergy == ALIGN_TOP) {
                offsetY = 0.0;
            } else if (fs_in.f_TexAlignStratergy == ALIGN_BOTTOM) {
                offsetY = fs_in.f_QuadSize.y - scaledTexSize.y;
            } else { // 默认居中对齐
                offsetY = (fs_in.f_QuadSize.y - scaledTexSize.y) / 2.0;
            }
        ...
        {
            // 计算高度基准的缩放因子
            float finalScale = fs_in.f_QuadSize.y / actualTexSize.y;
            // 计算缩放后纹理的尺寸
            vec2 scaledTexSize = actualTexSize * finalScale;
            // 根据对齐策略，计算水平方向的偏移量
            float offsetX = 0.0;
            if (fs_in.f_TexAlignStratergy == ALIGN_LEFT) {
                offsetX = 0.0;
            } else if (fs_in.f_TexAlignStratergy == ALIGN_RIGHT) {
                offsetX = fs_in.f_QuadSize.x - scaledTexSize.x;
            } else {
                // 默认居中对齐
                offsetX = (fs_in.f_QuadSize.x - scaledTexSize.x) / 2.0;
            }
        ...
        case FORCE_FILL:
        {
            ...
            result.uv = fs_in.v_TexCoord * fs_in.f_UVScale;
            break;
        }
        case TILE_REPEAT:
        {
        ...
        case TILE_BASEWIDTH_REPEAT:
        {
            ...
            // 映射到层内实际区域
            result.uv = finalUV_normalized * fs_in.f_UVScale;
            break;
        }
        ...
        case SINGLE:
        {
            ...
            if (fs_in.f_TexAlignStratergy == ALIGN_LEFT) {
                ...
                offsetX = fs_in.f_QuadSize.x - actualTexSize.x;
            } else {
                // 默认水平居中
                offsetX = (fs_in.f_QuadSize.x - actualTexSize.x) / 2.0;
            }
            ...
            if (fs_in.f_TexAlignStratergy == ALIGN_TOP) {
                ...
                offsetY = fs_in.f_QuadSize.y - actualTexSize.y;
            } else { // 默认垂直居中
                offsetY = (fs_in.f_QuadSize.y - actualTexSize.y) / 2.0;
            }
        ...
        case CHARACTER:
        {
            ...
            // 最终UV = 偏移量 + 插值坐标 * 尺寸
            result.uv = fs_in.f_Radius + (fs_in.v_TexCoord * fs_in.f_UVScale);
            break;
        }
    }
    return result;
...
void main() {
    ...
    vec4 finalColor;
    if (fs_in.f_TextureLayerIdx == -1) {
        finalColor = fs_in.f_DefColor;
    } else {
        // 先计算UV并判断可见性
        ...
        if (fs_in.f_TexScaleStratergy == CHARACTER) {
            // 对于单通道位图字体:
            // 从R通道获取字符的覆盖率/Alpha值
            float coverage = texture(u_samplerarray, vec3(uvResult.uv, fs_in.f_TextureLayerIdx)).r;

            // 使用 f_DefColor 作为基础颜色，并用覆盖率调制其Alpha通道
            finalColor = vec4(fs_in.f_DefColor.rgb, fs_in.f_DefColor.a * coverage);
        } else {
            // 对于所有其他RGBA纹理模式，使用原有逻辑
            finalColor = texture(u_samplerarray, vec3(uvResult.uv, fs_in.f_TextureLayerIdx)) * fs_in.f_DefColor;
        }
    }
    ...
    // --- 圆角和渐变效果处理 ---
    if (fs_in.f_Radius.x > 0.0 || fs_in.f_Radius.y > 0.0) {
        // 计算绘制区域的总尺寸 (包含了外部效果区域)
        vec2 drawingAreaSize = fs_in.f_QuadSize + 2.0 * fs_in.f_RadiusEffectParam;

        // 计算当前片元相对于绘制区域中心点的坐标
        vec2 pos = (fs_in.v_TexCoord - 0.5) * drawingAreaSize;

        // 计算圆角的像素半径
        // f_Radius 是一个比例值, 乘以矩形半高/半宽得到实际像素半径
        vec2 radiusInPixels = fs_in.f_Radius * (fs_in.f_QuadSize / 2.0);

        // 计算到圆角矩形的SDF距离
        float dist = sdRoundedBox(pos, fs_in.f_QuadSize / 2.0, radiusInPixels);

        // 根据SDF距离和效果类型，计算最终的 alpha 值
        if (fs_in.f_RadiusEffect == FADE_IN_AND_OUT) {
            // 使用 smoothstep 实现从边缘(dist=0)到外部渐变范围(dist=f_RadiusParams)的平滑过渡
            // 当 dist 从 0 -> f_RadiusParams, alpha 从 1 -> 0
            float fade_alpha = smoothstep(fs_in.f_RadiusEffectParam, 0.0, dist);
    ...
    if (fs_in.f_NoFilter == 0) {
        // 应用蒙版
        for (int mask_index = 0; mask_index < u_ActiveMaskLayerCount; ++mask_index) {
            MaskLayer mask = u_MaskStack[mask_index];
            bool is_inside_mask =
                fs_in.v_WorldPos.x > mask.rect.x && fs_in.v_WorldPos.x < mask.rect.z &&
                    fs_in.v_WorldPos.y > mask.rect.y && fs_in.v_WorldPos.y < mask.rect.w;
