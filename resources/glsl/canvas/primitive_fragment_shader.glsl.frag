...
UVResult calcUV() {
    ...
    if (fs_in.f_TexScaleStratergy != CHARACTER && (fs_in.f_UVScale.x == 0. || fs_in.f_UVScale.y == 0. || fs_in.f_GroupSize.x == 0. || fs_in.f_GroupSize.y == 0.)) {
    ...
    switch (fs_in.f_TexScaleStratergy) {
        case AUTO_SCALE_AND_CUT:
        {
            ...
            // 计算居中对齐的偏移量
            vec2 offset = (fs_in.f_QuadSize - scaledTexSize) / 2.;
            // 计算在源纹理上的采样像素坐标
        ...
        case SCALE_FORCE_BASEWIDTH:
        {
            ...
            if (fs_in.f_TexAlignStratergy == ALIGN_TOP) {
                ...
                offsetY = (fs_in.f_QuadSize.y - scaledTexSize.y) / 2.;
            }
        ...
        case SCALE_FORCE_BASEHEIGHT:
        {
            ...
            if (fs_in.f_TexAlignStratergy == ALIGN_LEFT) {
                ...
                // 默认居中对齐
                offsetX = (fs_in.f_QuadSize.x - scaledTexSize.x) / 2.;
            }
        ...
        case SINGLE:
        {
            ...
            if (fs_in.f_TexAlignStratergy == ALIGN_LEFT) {
                ...
                // 默认水平居中
                offsetX = (fs_in.f_QuadSize.x - actualTexSize.x) / 2.;
            }
            ...
            if (fs_in.f_TexAlignStratergy == ALIGN_TOP) {
                ...
                offsetY = (fs_in.f_QuadSize.y - actualTexSize.y) / 2.;
            }
...
void main() {
    ...
    // --- 圆角和渐变效果处理 ---
    if (fs_in.f_Radius.x > 0. || fs_in.f_Radius.y > 0.) {
        // 计算绘制区域的总尺寸 (包含了外部效果区域)
        vec2 drawingAreaSize = fs_in.f_QuadSize + 2. * fs_in.f_RadiusEffectParam;

        // 计算当前片元相对于绘制区域中心点的坐标
        vec2 pos = (fs_in.v_TexCoord - .5) * drawingAreaSize;

        // 计算圆角的像素半径
        // f_Radius 是一个比例值, 乘以矩形半高/半宽得到实际像素半径
        vec2 radiusInPixels = fs_in.f_Radius * (fs_in.f_QuadSize / 2.);

        // 计算到圆角矩形的SDF距离
        float dist = sdRoundedBox(pos, fs_in.f_QuadSize / 2., radiusInPixels);
        ...
        if (fs_in.f_RadiusEffect == FADE_IN_AND_OUT) {
            ...
            float fade_alpha = smoothstep(fs_in.f_RadiusEffectParam, 0., dist);
